...
void ADXL345::setFifoMode(int mode, int samples, int trigger_int){
    byte _s;

    switch (mode) {
        case ADXL345_FIFO_BYPASS:
            _s = B00000000;
            break;
        case ADXL345_FIFO_FIFO:
            _s = B01000000;
            break;
        case ADXL345_FIFO_STREAM:
            _s = B10000000;
            break;
        case ADXL345_FIFO_TRIGGER:
            _s = B11000000;
            break;
        default:
            _s = B01000000;
    }
    if(trigger_int == ADXL345_FIFO_INT2){
        _s |= B00100000;
    }
    _s |= (samples & B00011111);
    writeTo(ADXL345_FIFO_CTL, _s);
}

int ADXL345::getFifoMode(){
    byte _b;
    readFrom(ADXL345_FIFO_CTL, 1, &_b);
    return _b;
}

int ADXL345::getFifoStatusSampleCount(){
    byte _b;
